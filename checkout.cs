...
namespace OB455_Booklist
{
    public partial class Payment : Page
    {
        ...
        public void ShowHidePaymentMethods(DataTable DealerPaymentDetails)
        {
            ...
            if (((!string.IsNullOrEmpty(Convert.ToString(DealerPaymentDetails.Rows[0]["AFTERPAY_MERCHANT_ID"]))) && (!string.IsNullOrEmpty(Convert.ToString(DealerPaymentDetails.Rows[0]["AFTERPAY_SECRET_KEY"])))))
            {
                opt5.Style.Add("display", "table-cell");
            }
            else
            {
                opt5.Style.Add("display", "none");
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (RB5.Checked == true)
                {
                    ...
                    showhidePaypal.Style.Add("display", "none");
                    showhideCreditCard.Style.Add("display", "none");
                    showAfterpay.Style.Add("display", "block");
                }
                else
                {
                    opt1.Style.Add("display", "none");
                ...
                if (ShoppingCartPrdCount > 0 && IsRecaptchaVerified())
                {
                    if (Convert.ToInt32(hdDelMode.Value) == 1 || Convert.ToInt32(hdDelMode.Value) == 2 || Convert.ToInt32(hdDelMode.Value) == 4)
                    {
                      ...
                      /*Afterpay*/
                        ...
                        {
                            Session["TenderID"] = "Afterpay";
                            btnSubmitOrder.Style.Add("display", "none");
                            BtnLoader.Style.Add("display", "block");
                            AfterPayCheckout(orderId);
                        }
                    }
                    else
                    {
        ...
        /*Afterpay*/
        protected async void AfterPayCheckout(string orderId)
        {
            try
            {
                // Hardcoded Afterpay API credentials
                string username = "45776"; // Merchant ID
                string password = "4a9a9ec0e2a37646a94765e61d716fb3619849a977d6672bd1f9cdfb82f4d172515f9030a595dfac2bfe7fe96a3e480a42ff81dfcd1c2b582dccc64ca6bdb462"; // Secret key

                // Afterpay API URL (sandbox for testing)
                ...
                using (var client = new HttpClient())
                {
                    ...
                    // Hardcoded payload for Afterpay
                    var data = new
                    {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        string afterpayRedirectUrl = responseObject.redirectCheckoutUrl;

                        // JavaScript redirect
                        string script = $"window.location.href = '{afterpayRedirectUrl}';";
                        ScriptManager.RegisterStartupScript(this, GetType(), "redirectScript", script, true);
                    }
                    else
                    {
                        // Log error using Debug.WriteLine
                        Debug.WriteLine("Error processing Afterpay request: " + response.StatusCode);
                        string errorMessage = "Error processing Afterpay request. Please try again.";
            ...
            catch (Exception ex)
            {
                ...
                string script = $"alert('Exception occurred: {ex.Message}');";
                ScriptManager.RegisterStartupScript(this, GetType(), "errorAlert", script, true);
            }
        }
...
string returnURL = shost + "/" + URLKeyword + "/Order/ZipCheckout";
string shost = HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority);
string returnURL = shost + "/"+ storevalue + "/Order/Payment";
string cancelURL = shost + "/" + storevalue + "/View/Cart";
*/